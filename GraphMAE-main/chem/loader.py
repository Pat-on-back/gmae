...
# 把所有样本预加载到RAM中，用索引切片访问 所有图拼成一个大图张量与切片索引表
# 内部实际存储 
# data    → 拼接后的大张量
# slices  → 切片索引
...
def mol_to_graph_data_obj_simple(mol):
    """
    将 RDKit 的分子对象（mol object）转换为 PyTorch Geometric 包所需的图数据对象（Graph Data object）。
    注意：此处使用了简化的原子和键特征，并将其表示为索引。
    :param mol: RDKit 分子对象
    :return: 包含以下属性的图数据对象：x（节点特征）, edge_index（边索引）, edge_attr（边特征）
    """
    # atoms
...
class MoleculeDataset(InMemoryDataset):
    def __init__(self,
                 ...
                 #data = None,
                 #slices = None,
                 transform=None, # 每次读取时执行的动态变换
                 pre_transform=None, # 预处理时执行的动态变换
                 pre_filter=None, # 预处理筛选函数
                 dataset='zinc250k',
                 empty=False):
        """
        改编自 qm9.py。禁用了下载功能。
       :param root: 数据集所在的目录，包含一个原始数据目录（raw）和一个处理后数据目录（processed）。
        原始数据目录应包含存储 SMILES 字符串的文件；处理后数据目录可以为空，也可以包含之前已处理好的文件。
       :param dataset: 数据集名称。目前仅支持以下数据集：zinc250k, chembl_with_labels, tox21, hiv, 
        bace, bbbp, clintox, esol, freesolv, lipophilicity, muv, pcba, sider, toxcast。
       :param empty: 如果为 True，则不会加载任何数据对象。用于初始化一个空的数据集。
        """
    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            input_path = self.raw_paths[0] # 读取原始文件路径
            # 所有列强制读取为字符串，dtype=str防止 ZINC id 被自动转成整数丢失前导零
        ...
        # 预过滤器，删除不符合条件的图，如过滤掉原子数>50的分子
        if self.pre_filter is not None:
            data_list = [data for data in data_list if self.pre_filter(data)]

        # 预变换器，对每个图做额外特征变换，如计算节点度、加 Laplacian PE、添加子图信息
        if self.pre_transform is not None:
            data_list = [self.pre_transform(data) for data in data_list]
        data_smiles_series = pd.Series(data_smiles_list) # 把列表变成 pandas 序列
        # 写入processed/smiles.csv
        data_smiles_series.to_csv(os.path.join(self.processed_dir,
                                               'smiles.csv'), index=False,
                                  header=False)
        # data拼接后的大 Data 对象，slices每个样本边界索引
        # InMemoryDataset.collate()将多个图对象压缩存储成一个统一张量数据库，并记录索引以便随时恢复单图。
        data, slices = self.collate(data_list)
        # 保存处理结果，写入processed/data.pt
